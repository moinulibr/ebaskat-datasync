...
<input type="hidden" id="headerdata" value="{{ __('Merchant Details Report') }}">

<div class="content-area">
    <div class="mr-breadcrumb">
        <div class="row">
            <div class="col-lg-5">
                <h4 class="heading">{{ __('Merchant Details Report') }}</h4>
                <ul class="links">
                    <li>
                    ...
                    <li>
                        {{ __('Merchant Details Report') }}
                    </li>
                </ul>
            </div>
            <div class="col-md-7 mt-3">
                <div class="btn-group float-right" role="group" aria-label="Order Status">
                    <a href="{{ route('admin.report.merchant.details') }}" class="btn btn-info">Merchant Details</a>
                    <a href="{{ route('admin.report.merchant.subscription_history') }}" class="btn btn-secondary">Subscription history</a>
                    {{-- <a href="{{ route('admin.report.merchant.product_history') }}" class="btn btn-secondary">Product history</a> --}}
                            ...
                            <table id="report_table" class="table table-hover dt-responsive" cellspacing="0"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('Reg number') }}</th>
                                        <th>{{ __('Shop message') }}</th>
                                        <th>{{ __('Shop details') }}</th>
                                        <th>{{ __('Status') }}</th>
                                    </tr>
                                </thead>
                            </table>
...
@push('scripts')
    ...
    <script>
        let table = $('#report_table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('admin.report.merchant.details') }}",
                data: function(data){
