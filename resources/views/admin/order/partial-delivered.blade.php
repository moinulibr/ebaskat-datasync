...
@section('content')
    ...
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-4">
                    <h4 class="heading">{{ __('Partial Delivered Orders') }}</h4>
                    <ul class="links">
                        <li>
                        ...
                        <li>
                            <a href="{{ route('admin.order.partial_delivered') }}">{{ __('Partial Delivered Orders') }}</a>
                        </li>
                    </ul>
                </div>
                <div class="col-md-8 mt-3">
                    <div class="btn-group float-right" role="group" aria-label="Order Status">
                        <a href="{{ route('admin.order.index') }}" class="btn btn-secondary">All Orders</a>
                        @foreach (main_orders_status_hh() as  $index => $item)
                            @if ($index != 'partial_delivered')
                                <a href="{{ route('admin.order.'.$index) }}" class="btn btn-secondary">{{$item}}</a>  
                            @endif
                        @endforeach
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        var table = $('#geniustable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin.order.datatables','partial delivered') }}',
            columns: [
