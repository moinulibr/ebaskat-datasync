...
<div class="content-area">
	...
	<div class="add-product-content">
		<div class="row">
			...
			<div id="divReloadWithoutRefresh">
			</div>
				...
				<div class="gocover" style="background: url({{asset('assets/images/xloading.gif')}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
					<form id="" action="{{route('admin-prod-importsubmit')}}" class="submitCsvFile" method="POST"  enctype="multipart/form-data">
					{{csrf_field()}}  {{--class="submitCsvFile"--}}

					@include('includes.form-both')

						<div class="row">
							<div class="col-lg-12 text-right">
								<span style="margin-top:10px;"><a class="btn btn-primary" href="{{asset('assets/csv-upload-format/product-csv-format.csv')}}">{{ __("Download Sample CSV") }}</a></span>
							</div>
						</div>
						<hr>
	...
	<input type="hidden" value="{{route('admin.processing.bulk.product.upload.progress.bar')}}" class="adminProcessingBulkProductUploadProgressBar"/>
@endsection

@section('scripts')

<script src="{{asset('assets/admin/js/product.js')}}"></script>
<script src="{{asset('custom_js/bulk-product/bulk_product_upload.js')}}"></script>
