...
<div class="content-area">
			...
			<div class="col-lg-12 p-5">
				<div class="gocover" style="background: url({{asset('assets/images/xloading.gif')}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
					...
					@include('includes.form-both')
						...
						<div class="row">
							<div class="col-lg-12 mt-4 text-center">
								<button class="mybtn1 mr-5 disabledAttr import" type="submit" disabled>{{ __("Import Product From Bigbuy") }}</button>
							</div>
						</div>
				</div>
		</div>
	...
	<input type="hidden" value="{{route('adminImportProductFromBigbuy')}}" class="adminImportProductFromBigbuy"/>
	<input type="hidden" value="{{route('admin.dropship.bigbuy.product.import.progress.bar')}}" class="bigbuyProductImportProgressingBar"/>
	<input type="hidden" value="{{route('admin.dropship.bigbuy.update.value.after.completed.progress.bar')}}" class="updateInsertedValueAfterCompletedProgressBar"/>
@endsection

@section('scripts')

<script src="{{asset('custom_js/dropship/bigbuy.js')}}"></script>
