					...
					@include('includes.form-both')
						...
						<div class="row">
							<div class="col-lg-12 d-flex justify-content-center text-center">
								<strong class="currentPageNo" style="display: none;">Updating Page No : </strong> <strong class="current_page"></strong>
							</div>
						</div>
                        ...
                        <div class="row">
                            <div class="col-lg-12 mt-4 text-center">
                                <button class="mybtn1 mr-5 disabledAttr import" type="submit" disabled>{{ __("Update Product From Aliexpress") }}</button>
                            </div>
                        </div>
				</div>
		</div>
...
<input type="hidden" value="{{route('admin.dropship.aliexpress.update.imported.product')}}" class="aliexpressImportedProductUpdate"/>
<input type="hidden" value="{{route('admin.dropship.aliexpress.updated.value.for.product.update.progress.bar')}}" class="getUpdatedValueAfterUpdatingAliexpressProductForProgressBar"/>
<input type="hidden" value="{{route('admin.dropship.aliexpress.update.value.after.completed.progress.bar.for.product.update')}}" class="updatedValueAfterUpdateCompletedProgressBar"/>
@endsection

@section('scripts')
<script src="{{asset('custom_js/dropship/aliexpress-product-update.js')}}"></script>
@endsection
