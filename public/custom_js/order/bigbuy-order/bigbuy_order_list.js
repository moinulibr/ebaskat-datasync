    ...
    /*
    |--------------------------------------------------------
    | update delivery status when syncing individually 
    ...
    */
        ...
        $(document).on('click','.confirm_single_order_syning_button',function(e){
            e.preventDefault();
            statusAndSyncMessageProcessingHide();
    ...
    */
    ...
    /*
    |--------------------------------------------------------
    | update delivery status when syncing Bulking 
    | update order package delivery status
    |--------------------------------------------------------
    ...
    */
        ...
        $(document).on('click', '.confirm-syning-button', function (){
            var ids = [];
    ...
    */
    ...
    $(document).on('click','.adminOrderToBigbuy',function(e){
    ...
    $(document).on('click','.single-order-confirm-button',function(){ //adminOrderToBigbuy
        //e.preventDefault();
        $('.text-left-ul').html(" ");
        $('.text-left-li').html(" ");
    ...
    $()
    // single order place to Bigbuy end
    ...
    //bulk order place to Bigbuy (route for all checked order place to Bigbuy)
    //order place confirmation
    $(document).on('click','.allOrderPlaceToBigbuy',function(){
    ...
    $(document).on('click', '.confirm-button', function (){
        ...
        var url = $('.bulkOrderPlaceToBigbuy').val();
        if(ids.length <= 0) return ;
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(response.status == true)
                {
                    ...
                    $('.alertDanger').show();
                    $('.alert-danger').show();
                    $('.text-left').text('Order not process to bigbuy');
                }
            },
            complete:function(){
                $('.loading').fadeOut();
    ...
    //bulk order place to Bigbuy end
    ...
    */
    function statusLabel(status)
    {
        ...
        {
            $('.status_label').text('All Bigbuy Orders');
            status = "";
        }else{
    ...
    */
    ...
    |----------------------------------------------------------------
    */
        $(document).on('click','.bigbuyOrderNumberUpdate',function(){
            $('.bigbuyOrderIdErrorMessage').text('');
            $('.alertSuccessSingle').hide();
            $('.alert-success').hide();
            $('.text-left-single').text('');
            $('.displayOrderPackageNumber').text('');
            $('.orderPackageTableId').val('');
            $('.bigbuyOrderId').val('');
            var id  = $(this).data('id');
            var package_number = $(this).data('package_number');

            var aliex_order_id = $(this).data('aliex_order_id');

            $('#bigbuyOrderNumberUpdate_modal').modal('show');
            $('.displayOrderPackageNumber').text(package_number);
            $('.orderPackageTableId').val(id);
            $('.bigbuyOrderId').val(aliex_order_id);
        });
    /*
    |-----------------------------------------------------------------
    | bigbuy order number : open modal
    ...
    */
    ...
    |----------------------------------------------------------------
    */
        $(document).on('keyup','.bigbuyOrderId',function(){
            $('.bigbuyOrderIdErrorMessage').text('');
        });
        $(document).on('click','.updateBigbuyOrderNumberButton',function(){
            var orderPackageId  = $('.orderPackageTableId').val();
            var bigbuyOrderNo   = $('.bigbuyOrderId').val();
            
            if(!bigbuyOrderNo)
            {
                $('.bigbuyOrderIdErrorMessage').text('Bigbuy Order No/Number field is empty');
                return ;
            }
            var url = $('.bigbuyOrderIdUpdateUrl').val();
            $.ajax({
                url: url,
                type: "GET",
                datatype:"HTML",
                data:{
                    orderPackageId:orderPackageId,bigbuyOrderNo:bigbuyOrderNo
                },
                beforeSend:function(){
                    $('.loading').show()
                },
                success: function(response){
                    if(response.status == true)
                    {
                        ...
                        $('.alert-success').show();
                        $('.text-left-single').text(response.message);
                        $('#bigbuyOrderNumberUpdate_modal').modal('hide');
                        defaultLoading();
                    }
                },
                complete:function(){
